...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the infinity model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
...
function infinityremote()
{
    ...
    local INFINITY_USER=$(git config --get review.review.infinity.org.username)
    if [ -z "$INFINITY_USER" ]
    then
        git remote add infinity ssh://review.infinity.org:29418/$PFX$PROJECT
    else
        git remote add infinity ssh://$infinity_USER@review.infinity.org:29418/$PFX$PROJECT
    fi
...
function githubremote()
{
    ...
    git remote add github https://github.com/ProjectInfinity-X/$PROJECT
...
function infinityrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "Infinity Gerrit Rebase Usage: "
    ...
    git fetch "http://review.infinity.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
