function __print_infinity_functions_help() {
cat <<EOF
...
- infinitygerrit:   A Git wrapper that fetches/pushes patch from/to LineageOS Gerrit Review.
- infinityrebase:   Rebase a Gerrit change and push it again.
- infinityremote:   Add git remote for LineageOS Gerrit Review.
...
function breakfast()
{
    target=$1
    local variant=$2
    local aosp_target_release=$(cat ${ANDROID_BUILD_TOP}/vendor/infinity/vars/aosp_target_release 2>/dev/null)
...
function infinityremote()
{
    ...
    git remote rm infinity 2> /dev/null
    ...
    local INFINITY_USER=$(git config --get review.review.infinityos.org.username)
    if [ -z "$INFINITY_USER" ]
    then
        git remote add infinity ssh://review.infinityos.org:29418/$PFX$PROJECT
    else
        git remote add infinity ssh://$INFINITY_USER@review.infinityos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'infinity' created"
}
...
function makerecipe() {
    ...
    then
        pwd
        infinityremote
        git push infinity HEAD:refs/heads/'$1'
    fi
    '
}

function infinitygerrit() {
    ...
    local user=`git config --get review.review.infinityos.org.username`
...
EOF
            ...
            case $1 in
                ...
                changes|for)
                    if [ "$FUNCNAME" = "infinitygerrit" ]; then
...
function infinityrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        ...
        echo "      infinityrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.infinityos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $INFINITY_FIXUP_COMMON_OUT ]; then
