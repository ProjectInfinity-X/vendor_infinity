...
func init() {
	android.RegisterModuleType("infinity_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = infinityExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = infinityExpandVariables(ctx, dep_file)
	...
	cmd := infinityExpandVariables(ctx, String(g.properties.Cmd))
